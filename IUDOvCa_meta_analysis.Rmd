...
subset <- data[!is.element(data$author,c("Tworoger","Tuesley")),colidx] #omit this study
subset
```
...
```{r DataPrep}
subset <- subset[!is.na(subset$multiunspiud_est),]
subset <- subset %>% 
  mutate(
...
It appears that the effect estimate does not vary depending on the design of the study (good). We probaby want to combine procohort with nestedcase.

If we want to stratify the estimates by a study variable to look for heterogeneity, we would use the code that follows. 
