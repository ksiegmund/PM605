...
There are many ways to get biased estimates in a meta-analysis. One concern is publication bias. If investigators only publish findings that achieve statistical significance, only small studies with large effect sizes will achieve statistical signficance.  Including this biased subset of small studies can inflate the effect size away from the null. Small clinical studies might have biased effect sizes due to selection bias of the patients.  Only patients thought to be benefitted by a drug will be included. A series of methods are proposed to evaluate bias due to small studies.
...
Some properties of the regression line in this figure:
1. Var($y_k$) = 1.
...
If there are no small-study effects (biases), the studies are expected to scatter randomly about this line through the origin.

####  Egger Test

A common test for asymmetry is Egger's test.  For this, we fit a regression line to the data from the radial plot, and test if the line goes through the origin.
...
The different slopes between the solid line (best line fit) and line through the origin (yielding the fixed effect estimate) is due to asymmetry of funnel plot. 
...
Estimates from the fixed effect and random effects model are much more similar for the trim-and-fill model. This is because the small samples with large effect were given more weight in the original REs model allowing them to estimate a larger effect size. Now that their contributions are balanced with the filled-in studies, the random effects and FE models have more similar estimates.

###  4. Cummulative Forest Plot

Let's review perform more bias assessment using a cummulative forest plot. We will order these by standard error
