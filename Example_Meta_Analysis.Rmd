...
mb <- metagen(TE=lnes,seTE=selnes, 
               sm = "OR", method.tau = "DL", backtransf = TRUE,
...
Instead of just the fixed effects model, we also get the summary from the random effects model.  The output tells us that it uses the DerSimonian-Laird estimator for tau^2 in the random effects model. The summary OR estimate from the random effects model is 0.57 (95\% CI: 0.36-0.91) ( p = 0.0177 ). 
...
```{r FixedEffect}
mb1 <- metabin(Tevents, TN, Cevents, CN, sm="OR", method.tau = "DL",
...
Aside: The DerSimonian-Laird estimator estimates tau^2 by direct calculation, which is convenient if working with Excel or a calculator. The restricted maximum likelihood (REML) estimator, is the default method in R and cannot be computed by direct calculation.
```{r FixedEffect}
...
Using the REML estimator changes slightly our estimate of the summary OR  from the random effects model ( OR = 0.56 (95\% CI: 0.36-0.89),  p = 0.0139 vs OR = 57 (95\% CI: 0.36-0.91),  p = 0.0177 using the DerSimonian-Laird estimator). 
